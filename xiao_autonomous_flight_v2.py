...
# === GLOBAL VARIABLES ===
# === XIAO AUTONOMOUS GLOBALS ===
camera_active = False
camera_last_yaw = 0.0
camera_last_coll = 0.0
camera_last_flags = 0
autonomous_mode_active = False
AUTONOMOUS_FORWARD_VX = 0.2
AUTONOMOUS_YAW_GAIN = 1.5
AUTONOMOUS_YAW_CLAMP = 45.0
...
def uartcam_callback(timestamp, data, logconf):
    global camera_active, camera_last_yaw, camera_last_coll, camera_last_flags
    camera_active = bool(data.get("uartcam.active", 0))
    camera_last_yaw = data.get("uartcam.lastYaw", 0.0)
    camera_last_coll = data.get("uartcam.lastColl", 0.0)
    camera_last_flags = data.get("uartcam.lastFlags", 0)
...
def setup_logging(cf):
    """Setup motion sensor, battery voltage, and motor logging"""
    ...
    try:
        ...
        added_motor_vars = []
        for var_name, var_type in motor_variables:
            group, name = var_name.split('.')
            if group in toc and name in toc[group]:
                try:
                    log_motors.add_variable(var_name, var_type)
                    added_motor_vars.append(var_name)
                except Exception as e:
                    print(f"Failed to add motor variable {var_name}: {e}")
            else:
                print(f"Motor variable not found: {var_name}")
        ...
        if len(added_motor_vars) > 0:
            log_motors.data_received_cb.add_callback(motor_callback)
        ...
        if len(added_motor_vars) > 0 and not log_motors.valid:
            print("WARNING: Motor log configuration invalid!")
            log_motors = None
        ...
        # Setup UARTCam logging
        log_cam = LogConfig(name="UARTCam", period_in_ms=100)
        added_cam_vars = False
        try:
            log_cam.add_variable("uartcam.active", "uint8_t")
            log_cam.add_variable("uartcam.lastYaw", "float")
            log_cam.add_variable("uartcam.lastColl", "float")
            log_cam.add_variable("uartcam.lastFlags", "uint8_t")
            log_cam.data_received_cb.add_callback(uartcam_callback)
            cf.log.add_config(log_cam)
            log_cam.start()
            added_cam_vars = True
            print("Added UARTCam logging.")
        except Exception as e:
            print(f"Failed to setup UARTCam logging: {e}")
            log_cam = None

        print(f"Logging started - Motion: {len(added_motion_vars)}, Battery: {len(added_battery_vars)}, Motors: {len(added_motor_vars)}")
        return log_motion, log_battery, log_motors
...
class DeadReckoningGUI:
    ...
    def create_ui(self):
        ...
        self.autonomous_button = tk.Button(control_frame, text="Start Autonomous",
                                     command=self.toggle_autonomous, bg="#2196F3", fg="white", font=("Arial", 12))
    ...
    def reset_all_values(self):
        ...
        self.trim_vx_var.set("0.0")
        self.trim_vy_var.set("0.0")
    ...
    def toggle_autonomous(self):
        global autonomous_mode_active, flight_active
        if not flight_active:
            self.status_var.set("Status: Start flight first!")
            return
        autonomous_mode_active = not autonomous_mode_active
        if autonomous_mode_active:
            self.autonomous_button.config(text="Stop Autonomous", bg="#FF9800")
            self.status_var.set("Status: AUTONOMOUS MODE ON")
        else:
            self.autonomous_button.config(text="Start Autonomous", bg="#2196F3")
            self.status_var.set("Status: AUTONOMOUS MODE OFF")
    ...
    def flight_controller_thread(self):
        ...
        try:
            ...
            with SyncCrazyflie(DRONE_URI, cf=cf) as scf:
                ...
                # Setup CSV Logging
                csv_dir = r"D:\DRONE\Position hold"
                if not os.path.exists(csv_dir):
                    os.makedirs(csv_dir)
                
                timestamp_str = datetime.now().strftime("%Y%m%d_%H%M%S")
                csv_filename = os.path.join(csv_dir, f"flight_log_{timestamp_str}.csv")
                csv_file = open(csv_filename, mode='w', newline='')
                csv_writer = csv.writer(csv_file)
                
                # Write header
                csv_writer.writerow([
                    "Timestamp", "Flight Phase", 
                    "Battery (V)", "Height (m)",
                    "Motion dX", "Motion dY",
                    "Velocity X", "Velocity Y",
                    "Pos X", "Pos Y",
                    "PID Corr VX", "PID Corr VY",
                    "Command VX", "Command VY",
                    "Motor 1", "Motor 2", "Motor 3", "Motor 4",
                    "Cam Active", "Cam Yaw", "Cam Coll", "Cam Flags"
                ])
                
                start_log_time = time.time()
                
                def log_data(phase, cmd_vx=0.0, cmd_vy=0.0):
                    t = time.time() - start_log_time
                    csv_writer.writerow([
                        f"{t:.3f}", phase,
                        f"{current_battery_voltage:.2f}", f"{current_height:.3f}",
                        motion_delta_x, motion_delta_y,
                        f"{current_vx:.4f}", f"{current_vy:.4f}",
                        f"{integrated_position_x:.4f}", f"{integrated_position_y:.4f}",
                        f"{current_correction_vx:.4f}", f"{current_correction_vy:.4f}",
                        f"{cmd_vx:.4f}", f"{cmd_vy:.4f}",
                        motor_m1, motor_m2, motor_m3, motor_m4,
                        int(camera_active), f"{camera_last_yaw:.2f}", f"{camera_last_coll:.2f}", camera_last_flags
                    ])
                    csv_file.flush()

                # Setup logging
                flight_phase = "SETUP"
                log_motion, log_battery, log_motors = setup_logging(cf)
                ...
                while time.time() - start_time < TAKEOFF_TIME and flight_active:
                    ...
                    log_data(flight_phase, TRIM_VX, TRIM_VY)
                    time.sleep(0.01)
                ...
                while time.time() - start_time < HOVER_DURATION and flight_active:
                    ...
                    if autonomous_mode_active:
                        collision_detected = bool(camera_last_flags & 0x01) or (camera_last_coll > 0.7)
                        if collision_detected or not camera_active:
                            vx = 0.0
                            yawrate = 0.0
                        else:
                            vx = AUTONOMOUS_FORWARD_VX
                            yawrate = camera_last_yaw * AUTONOMOUS_YAW_GAIN
                            yawrate = max(-AUTONOMOUS_YAW_CLAMP, min(AUTONOMOUS_YAW_CLAMP, yawrate))
                        
                        if not DEBUG_MODE:
                            cf.commander.send_hover_setpoint(vx, 0.0, -yawrate, TARGET_HEIGHT)
                        log_data(flight_phase, vx, 0.0)
                    else:
                        if not DEBUG_MODE:
                            cf.commander.send_hover_setpoint(total_vx, total_vy, 0, TARGET_HEIGHT)
                        log_data(flight_phase, total_vx, total_vy)
